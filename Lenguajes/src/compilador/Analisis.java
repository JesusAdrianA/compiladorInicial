	...
	public Token analisisSemantico(NodoDoble<Token> nodo) {
		...
		if(nodo!=null) {
			to =  nodo.dato;
				...
				if (to.getValor().equals("string")) {	
					if(!(nodo.siguiente.siguiente.dato.getValor().contains(";"))) {
						if(!isString(nodo.siguiente.siguiente.siguiente.dato.getValor())) {
							System.out.println("Entro al primer String");
							String guarda;
							guarda=poderoso(nodo.siguiente.siguiente.siguiente.dato.getValor());
							impresion.add("Error semantico en la linea "+to.getLinea()+" al asignar a '"+nodo.siguiente.dato.getValor()+"' del tipo String"+" el valor '"+ nodo.siguiente.siguiente.siguiente.dato.getValor()+"' del tipo de dato "+guarda);
						}
					}
